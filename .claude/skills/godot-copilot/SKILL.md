...
**开发者（你）负责：**
- **<font color="red">场景开发(.tscn)的手动创建</font>** - AI助手无法操作场景文件
- **<font color="red">Godot项目配置(project.godot)</font>** - AI助手禁止修改项目设置
- 动画编排
- UI设计和美术资源准备
- 测试执行和功能验证
- 音频资源准备
...
**⚠️ 重要提醒：**
- AI助手（我）被严格禁止修改 project.godot 和任何 .tscn 文件
- 每当需要修改这些文件时，我会用红色字体明确提醒你
- 请按照提供的详细步骤手动完成这些操作
...
### 第一条：职责划分（🚨 绝对禁止条款）
- **AI助手（我）绝对禁止修改项目设置文件(project.godot)**
  - 禁止读取、写入、编辑 project.godot 文件
  - 禁止修改项目配置、依赖项、输入映射等
  - 必须提醒用户手动进行项目配置修改

- **AI助手（我）绝对禁止创建/修改/删除任何场景文件(.tscn)**
  - 禁止创建新场景文件
  - 禁止编辑现有场景文件（包括节点结构调整、属性修改等）
  - 禁止删除场景文件
  - 必须用橙色字体明确提醒用户手动创建/修改场景文件

- **AI助手（我）绝对禁止修改项目根目录下的任何配置文件**
  - 禁止修改 .gdignore、.import 等配置文件
  - 禁止修改任何 Godot 引擎相关的配置

- **工作流程强制要求**
  - 必须等待开发者（你）完成场景文件(.tscn)和项目配置(project.godot)后
  - AI助手（我）只能开发 gdscript 脚本文件(.gd)
  - 任何时候需要修改上述被禁止的文件时，必须明确提醒用户手动操作
...
- **必须使用** `godot` MCP工具进行场景创建和项目管理
  - ⚠️ 注意：仅用于提供指导，实际场景文件(.tscn)必须由用户手动创建
...
### 🚨 重要禁止事项（宪法级）
AI助手（我）**严格禁止**以下操作：
1. **禁止修改 project.godot 文件** - 任何项目配置更改必须提醒用户手动操作
2. **禁止创建/修改 .tscn 场景文件** - 只能提供指导，让用户手动创建
3. **禁止修改任何 Godot 引擎配置文件** - 包括 .import、.gdignore 等
4. **禁止使用文件系统工具直接操作上述文件**

当需要上述操作时，必须：
- 用<font color="red">**红色文字**</font>明确提示用户
- 提供详细的操作步骤
- 等待用户完成后继续开发
