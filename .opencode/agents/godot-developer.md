...
- P1-15S Story 开发标准流程中的 S3（编写失败测试）、S4（编写最小实现）、S5（重构）步骤
...
- GDScript 文件必须定义 class_name（Autoload 除外，P1-20）
- 碰撞层/掩码禁止在代码中硬编码（P2-25）
...
- @onready 变量需手动赋值（P2-22）
- script.new() 实例禁止调用 move_and_slide()（P2-22）
- GUT -gdir 不会递归搜索，需显式指定目录（P2-22）
