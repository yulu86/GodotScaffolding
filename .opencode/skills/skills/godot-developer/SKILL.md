...
**架构 → 场景 → 实现**。接收 `godot-architect` 的设计方案，**优先完成场景开发**（由 `godot-scene` 负责），再通过 TDD 实现代码，使用 `godot-test` 和 `godot-debug` 验证。

### 场景优先原则

当 Story 同时涉及场景（`.tscn`）和脚本（`.gd`）开发时：
1. **必须**先完成场景开发（`godot-scene` 创建框架 + 用户可视化操作）
2. 用户确认场景完成后，方可启动脚本编写
3. 场景开发未完成前，禁止编写任何业务逻辑代码
...
- ✅ **允许**：创建和编辑 .gd 脚本文件
- ✅ **允许**：通过 MCP 工具读取项目信息
- ❌ **禁止**：直接修改 .tscn 场景文件（使用 `godot-scene`，且场景开发优先于脚本开发）
