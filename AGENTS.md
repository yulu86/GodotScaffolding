...
brave-adventure/
├── addons/
│   └── gdUnit4/                  # 测试框架插件（bin/ src/ plugin.cfg runtest.cmd/.sh）
├── assets/
│   ├── bus/                      # 音频总线资源（bus.tres）
│   ├── fonts/                    # 字体（PixelOperator8 / SmileySans-Oblique）
│   ├── music/                    # 背景音乐（forest / time_for_adventure）
│   ├── resources/                # 数据资源（sound_bus/ texture/ theme/ tileset/）
│   ├── sounds/                   # 音效（kenney_impact-sounds / Minifantasy_Dungeon_*）
│   ├── sprites/                  # 精灵（character/ gdb-gamepad-2/ Legacy-Fantasy_*/ Knight_Rank_1）
│   └── themes/                   # UI 主题（main_theme.tres + button stylebox）
├── docs/
│   ├── 00_scratch/               # 草稿素材（截图）
│   ├── 01_需求/01_核心玩法.md     # 核心玩法设计（GDD）
│   ├── 03_美术规范/01_美术资产清单.md
│   ├── 04_音频规范/01_音频资产清单.md
│   └── 99_postmortem/MEMORY.md   # 经验沉淀（强制维护，见 §12.6）
├── scenes/                       # 场景文件 .tscn（按模块分子目录）
├── scripts/                      # 脚本 .gd（与场景一一对应）
├── test/
│   ├── unit/                     # 单元测试
│   ├── integration/              # 集成测试（按模块分子目录）
│   └── functional/screenshots/   # 端到端测试 + 截图
├── AGENTS.md                     # 本文件（项目宪法）
├── icon.svg                      # 项目图标
