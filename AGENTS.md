...
可视化搭建 → AI 用 MCP 写视觉初值 + 出指导，用户在编辑器精调（§14.2）—— 最终视觉/手感以用户为准
```
...
### 12.5 `[P0]` 场景可视化职责分工（AI + 用户协作）

> **理由**：AI 无法"看见"渲染与物理结果，但可凭命名约定、§03 美术规范、设计文档给出**合理初值**；而"最终好不好看 / 手感对不对"只能靠人眼与试玩判定。故场景可视化采用「**AI 用 MCP/编辑器写入初值 → 用户精调到最终效果**」两段式：既避免 AI 从零瞎猜，又不让用户做重复性机械配置。

**AI 负责（逻辑/结构层 + 可视化初值，可自动化）**：
- 脚本代码 `.gd`：状态机、信号、业务逻辑、数据驱动。
- 场景节点树**骨架** `.tscn`：用 `【MCP】` `godot-mcp` 创建节点、父子层级、挂载脚本、设置 `%UniqueName`。
- **可视化属性初值**：在骨架上为视觉属性写入**合理默认值**（依据命名约定 / 美术规范 / 设计文档，非瞎猜）——如挂 `CollisionShape2D` + 选定形状 + 默认尺寸、按约定指定 `texture`、设默认 zoom/limit、挂 `GPUParticles2D` 默认参数、建空 `SpriteFrames` 资源与动画名。
- 数据资源 `.tres`：数值与配置。
- 测试代码与质量门禁。

**用户负责（可视化精调，必须眼看手调 / 试玩）**：

| 类别 | AI 已写入初值 | 用户精调到最终 |
|------|--------------|---------------|
| 动画 | 建 `SpriteFrames` 资源、创建动画名、设默认 fps/loop | 拖入实际精灵帧、按真实素材调帧序与 fps、autoplay 选择 |
| 碰撞 | 挂 `CollisionShape2D` + 选定形状 + 默认尺寸 | 按精灵实际外形精调形状尺寸/偏移/多边形顶点 |
| 变换 | 按设计文档设初始 position/scale/rotation | 试玩后微调位置、缩放、朝向 |
| 纹理 | 按命名约定指定 `texture`、设默认 hframes/vframes | 按实际精灵调 region/偏移/hframes |
| 摄像机 | 设默认 zoom、limit、smoothing | 试玩后精调取景范围/平滑系数 |
| 布局 | 用容器 + 锚点/边距初值搭结构 | 手调间距/对齐/最终视觉 |
| 瓦片 | 配好 `TileSet` 资源引用 | 实际绘制 TileMap 格子（纯视觉，AI 不能画） |
| 音频 | 按清单指定音频流、设默认音量/音调 | 试听后精调音量/音调/淡入淡出 |
| 粒子/特效 | 挂 `GPUParticles2D` + 默认参数 | 精调生命周期/颜色/密度等视觉参数 |

**交互契约**：AI 写入合理初值并触发「可视化搭建指导」流程（§14.2，含已写入初值清单 + 需精调项）；**用户精调结果以编辑器最终值为准，AI 不得回退覆盖**；精调中如需调整骨架/脚本/初值，用户在 `question` 中选"需 AI 调整"。
...
> **`[P0]` 任务启动前置**：每个任务开始前**必须**先读取 `docs/99_postmortem/MEMORY.md`，吸收历史经验教训，避免重复踩坑（§12.4）。
> **`[P0]` 开发收尾前置**：代码开发完成后、提交前**必须**在 Godot 编辑器中重新加载当前项目（Project → Reload Current Project，或退出后用 `$GODOT_HOME/godot -e` 重启编辑器），刷新资源导入状态与场景/脚本引用，避免 `.import` 与缓存漂移导致后续门禁/运行时错误。
> **`[P0]` 可视化协作前置**：涉及可见节点时，门禁前**必须**输出「可视化搭建指导」（§14.2）并用 `question` 暂停，等用户在初值基础上精调并反馈。
> **`[P0]` 玩家手工验证前置**：门禁通过 + 编辑器重载后，**必须**暂停并输出「玩家手工验证指导」（§14.1），用 `question` 工具等待玩家反馈；**验证通过方可提交，禁止跳过验证直接 commit**。

```
【前置·必做】读取 docs/99_postmortem/MEMORY.md 吸取经验教训（§12.4）

需求 →【Skill: architect】 架构设计文档(含 Mermaid 图)
     →【Skill: best-practices + TDD 小循环】 AI: 1个测试 → 最小实现 → 重构（循环）+【MCP】搭场景节点骨架
     →【可视化协作】AI 用 MCP 写视觉初值 + 输出「可视化搭建指导」+ question 暂停（§14.2）—— ⚠️ 涉及可见节点时强制，用户精调到最终
     ...
     →【收尾·必做】Godot 编辑器重新加载当前项目（Project → Reload Current Project，刷新资源导入与场景/脚本引用）
     →【玩家手工验证】输出验证指导 + question 等待玩家反馈（§14.1）—— ⚠️ 验证通过前禁止提交
...
「编辑器重载后、提交代码前」**必须**输出验证指导并用 `question` 等玩家反馈，通过后方可提交。**禁止**跳过，**禁止**用自动化测试替代。
...
| 部分 | 要求 | 示例 |
|------|------|------|
| ① 运行方式 | 启动**现有**游戏并正常触达功能（F5/F6/命令行，不另建场景） | "F5 运行 `res://scenes/main.tscn`，正常进入游戏" |
| ② 验收标准映射 | 列 story 的 AC1/AC2… 作为验证清单 | "AC1：可左右移动；AC2：碰墙无穿模" |
| ③ 验证步骤 | 针对②的逐步操作（全部走现有实现，按键给具体 Input Action） | "1. 按 `move_right`(→) 右移；2. 撞墙观察是否停止" |
| ④ 预期表现 | 每条 AC 的 Pass 判据（画面/动画/音效/数值） | "AC2：撞墙静止、坐标不再增大" |
| ⑤ 异常判定 | 判定失败的现象 + 玩家需提供的反馈（报错截图/日志/复现步骤） | "穿墙/红色报错/行为不符 AC2 → 失败，请截图 + 复现步骤" |

**反馈机制**：用 `question` 给两选项后**立即停止**：① ✅ 验证通过，继续提交（推荐）；② ❌ 有问题，需修复。
- 通过 → 提交代码。
- 不通过 → 修复循环（最小改动 → 重跑诊断/lint → 重载编辑器 → **再次**输出验证指导 + `question`），由玩家驱动，AI 不得擅自宣布通过。

### 14.2 `[P0]` 可视化搭建协作（强制关卡）

「AI 完成场景骨架 + 可视化初值后、门禁前」若涉及可见节点，**必须**输出可视化搭建指导并用 `question` 暂停，等用户在初值基础上完成精调并反馈，方可继续。**禁止**跳过，**禁止** AI 把"初值"当成"最终值"直接进入门禁。

> **分工依据**：见 §12.5。

**触发条件**（满足任一即触发本关卡）：
- 新建或修改含可见节点的场景：`Sprite2D` / `AnimatedSprite2D` / `CollisionShape2D` / `Camera2D` / `Control` / `TileMapLayer` / `AudioStreamPlayer` / `GPUParticles2D` 等。
- 需要配置动画、碰撞形状、节点变换、纹理、摄像机、布局、瓦片、音频、粒子等视觉属性。

**AI 先做（初配）**：用 `【MCP】` `godot-mcp` 为视觉属性写入合理初值（依据命名约定 / 美术规范 / 设计文档）。

**可视化搭建指导**至少含 5 部分：

| 部分 | 要求 | 示例 |
|------|------|------|
| ① 场景与节点 | 目标场景路径 + 涉及节点的完整路径与类型 | `res://scenes/player.tscn` → 节点 `Player/AnimatedSprite2D` |
| ② 已写入初值 | 列出 AI 已用 MCP 配好的属性与值（让用户知道起点） | `CollisionShape2D` → `CapsuleShape2D` 半径=16（已写入）；`Camera2D.zoom`=2.0（已写入） |
| ③ 需精调项 + 参考规格 | 逐项列出需用户精调的属性、参考值、为何需手调 | `AnimatedSprite2D` → 需拖入实际精灵帧建 `idle`/`run` 动画（AI 不知帧布局）；碰撞尺寸**需按实际精灵微调** |
| ④ 精调操作步骤 | 编辑器逐步操作（哪个面板、怎么拖拽、怎么设属性） | 1. 选中 `AnimatedSprite2D`；2. Inspector → Sprite Frames → 打开 `player_frames.tres`；3. 拖入帧、设 fps… |
| ⑤ 反馈要求 | 用户精调完成后需提供什么 + 如何反馈 | 回复"已完成"，或截图动画预览/碰撞形状供确认 |

**反馈机制**：用 `question` 给两选项后**立即停止**：
- ① ✅ 已完成精调，继续后续门禁（推荐）；
- ② ❌ 遇到问题/需要 AI 调整骨架、脚本或初值。

- 完成 → 进入质量门禁（§10）。
- 需调整 → AI 修骨架/脚本/初值 → 重新输出可视化搭建指导 → 再次暂停。

**与 §14.1 的区别**：
- 本关卡（§14.2）是**搭建期**配置：AI 写初值并出指导、用户在编辑器精调视觉属性。
- §14.1 是**运行期**验证：玩家试玩黑盒验收。
- 两者**不可互相替代**，缺一不可。
...
- ✅ 场景可视化分工（AI 用 MCP 写初值 + 用户在编辑器精调）—— §12.5 / §14.2
