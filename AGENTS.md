...
### 4.4 `[P0]` 碰撞层（Physics Layers）命名规范
...
- **例外说明**：碰撞层名属于引擎配置（`project.godot`），非代码标识符，故不受 §3.4「代码除注释外无中文」约束；脚本中以 `collision_layer` / `collision_mask` 写入的**位掩码值**仍按代码规范处理（优先用具名常量/枚举表达，注释可中文）。
...
需求 →【任务简报】总结任务概要/简报和验收标准
     →【Skill: wiki-query】优先检索本地 Wiki 知识库中可借鉴的 Godot 经验/技术点（§12.8）⚠️涉及 Godot 经验/技术时强制先行
...
### 12.8 `[P0]` Wiki 经验优先查询（Godot 知识检索）
...
- **任务完成后**：本次产生的可复用 Godot 经验，除按 §12.6 沉淀到 `MEMORY.md` 外，鼓励回填 Wiki 供后续任务检索。

**与 §12.6 的边界**：§12.8 是**开发前/中**的知识**检索与引用**（输入侧，避免臆造）；§12.6 是**任务后**的经验**沉淀与回填**（输出侧，避免流失）。两者互补，不可互相替代。
