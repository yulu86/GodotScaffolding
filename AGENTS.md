...
| P0-35 | 新增 .gd/图片/音频/.tscn 必跑 `--headless --import` 生成 .uid/.import | → D1 / E1 |
...
**命令入口**：完整命令见 **E1**（godot）；项目 Markdown 文档检索/索引走 `【Skill】` `qmd`（详见 C5）。
...
- **做什么**：读 `./MEMORY.md` 吸取教训；总结任务简报与验收标准；用 `【Skill】wiki-query` 检索本地 Wiki 中可借鉴的 Godot 经验（P0-30，详见 C4）。
...
每次任务/检视后，将经验追加到 `./MEMORY.md`，按**双区**区分：**通用经验**（跨 Godot 项目可复用，如"浮点用 assert_almost_eq"）、**项目专属经验**（仅适用本项目）。
...
## C5 `[P1]` 项目 Markdown 文档检索优先用 qmd Skill
...
**使用方式**：检索/读取项目 `.md` 文档时，**必须**先加载 `【Skill】` `qmd`，按 Skill 指导使用 qmd CLI（`qmd search`/`qmd query` 检索，`qmd get`/`qmd multi-get` 读取内容），而非直接用 `read`/`grep`/`glob` 多文件试错。**命令语法与检索技巧见 Skill**，本节仅约定**项目级配置与强制触发条件**。

**索引范围**：项目内全部 `.md` 文档，**强制排除** `.tmp/`、`addons/`、`.godot/`、`.qmd/`、`.opencode/`、`.zcode/`、`.git/` 等临时/第三方/工具目录。

**初始化（首次或新机器）**：按 `【Skill】` `qmd` 的配置与维护指引初始化本地索引（`.qmd/` 须 `.gitignore`，见 D1），并添加项目文档集合：

```bash
qmd collection add {this-project-name} "docs/,AGENTS.md,README.md"   # {this-project-name} 为占位符，需替换成当前项目名称
qmd embed -c {this-project-name}
```

**查询优先级**（满足任一触发，**必须**优先用 `【Skill】` `qmd` 而非 `grep`/`read` 多文件试错）：
...
### 资源导入与 UID/.import 生成（P0-35）

- **触发条件**：在 `res://` 下**新增**以下任意类型文件时**必须**执行 headless 导入，生成对应的 `.uid` / `.import` 文件：
  - **脚本**：`.gd`（生成 `.uid`，供 UID 引用，避免改名/移动导致断裂引用）
  - **图片**：`.png`/`.jpg`/`.svg`/`.webp` 等（生成 `.import` 纹理导入配置 + `.uid`）
  - **音频**：`.wav`/`.ogg`/`.mp3` 等（生成 `.import` 音频流导入配置 + `.uid`）
  - **场景/资源**：`.tscn` / `.tres`（生成 `.uid`）
- **必跑命令**（项目根目录执行，命令入口见 E1）：
  ```bash
  $GODOT_HOME --headless --import
  ```
- **强制要点**：
  - **新增即导入**：每次新增上述文件后**必须立即**跑一次导入，禁止把"未导入"的文件直接提交或交给后续流程（场景引用、`load()`、validate_scenes 等均依赖 `.import`/`.uid` 存在）。
  - **不得手写**：`.uid` / `.import` **禁止**手工编写，必须由引擎 `--headless --import` 生成（与 P0-23 `.tscn`/`.tres` 禁手写同理）。
  - **版本控制**：`.import` 文件**纳入**版本控制（确保 clone 即用、不依赖二次导入）；`.godot/` 缓存目录仍 `.gitignore`（见 P0-05）。
  - **编辑器重载关系**：`--headless --import` 与 B6/B7 前置的"编辑器重载"互补——新增资源后优先命令行导入，提交前再 `$GODOT_HOME -e` 重载确认。
- **后果警示**：若跳过导入，将出现 `load()` 失败、资源显示为空、`validate_scenes` 报断裂引用、移动文件后 UID 丢失等隐蔽错误。
...
> **MVP 最小必备集合（5 件）**：`README.md`（纯游戏主题）+ `00_开发指南/01_快速开始.md`（技术性内容归宿）+ `01_需求/01_核心玩法.md` + `02_架构/01_技术架构.md` 。其余 🟡 项须在对应功能实现前先出大纲，避免"先写码后补文档"。
...
| 检视/复盘 | 本宪法 D2–D10、`MEMORY.md` | 检视总结 | `./MEMORY.md` |
...
> **qmd**：项目 Markdown 文档检索/索引走 `【Skill】` `qmd`（命令语法见 Skill）；项目级配置与强制触发条件见 **C5**。
