...
>
> **确认模式**（见 P1-26）：默认 `连续式`——S1→S2→S3→S4 文档准备阶段连续产出，仅在 S4 末尾统一确认一次；S15 代码检视默认 AI 自检、异常项才复核；S17 场景验收默认截图自动比对。用户可切换为 `交互式`（每个确认点单独暂停）。

| 步骤 | 名称 | 代理 | 关键动作 |
|------|------|------|--------|
| S1 | 开发前准备 | 主代理 | 读 `MEMORY.md` → 输出 Story 概要（连续式：不单独暂停，进入 S2；交互式：暂停确认） |
...
| S15 | 代码检视 | `godot-reviewer` | 总结开发内容 → 在编辑器中逐个打开文件，说明职责、修改关键点、检视关键点 → **每检视完一个文件后展示检视进度**（示例：`检视进度: [███░░░░] 3/7`，`✓` 已完成、`→` 当前、其余为待检视） → **暂停等用户检视结果**（连续式：AI 自检通过则不逐文件暂停，仅异常项提请复核；交互式：逐文件暂停） |
...
- **P1-26** **确认模式**：Story 执行中 AI 暂停等用户的策略，默认 `连续式`，用户可随时切换为 `交互式`：
  - **连续式**：① S1→S2→S3→S4 文档准备阶段连续产出，仅在 S4 末尾统一确认一次；② S15 代码检视 AI 自检通过后不逐文件暂停，仅异常项提请用户复核；③ S17 场景验收用截图自动比对，通过则不暂停
  - **交互式**：每个确认点（S1/S2/S3/S4/S8/S9/S15/S16/S17）均单独暂停等用户
  - **切换**：用户在任一暂停点或任务中说出"切换交互式/连续式"即生效；Story 开始时默认连续式
  - **硬性暂停点（两模式均必须暂停，不因连续式跳过）**：S8（需用户在编辑器完成可视化操作）、S9/S16（出现一致性差异时）、Story 间确认（P1-2）。安全/质量红线步骤不得自动放行
...
- **P1-6** S15 代码检视：AI 逐文件展示变更摘要，与用户一起检视（连续式下 AI 自检通过则不逐文件暂停，仅异常项复核，见 P1-26）
