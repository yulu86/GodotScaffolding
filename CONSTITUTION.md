...
- **P0-11.5** 【触发时机：TDD 的 Red 阶段】**禁止**一次性编写所有类的测试用例。**必须**按类的依赖关系排序（依赖最少的类优先），每次**只编写一个类**的测试用例（Red），然后立即编写该类的最小实现代码使测试通过（Green），再进行重构（Refactor）。完成一个类的完整 Red→Green→Refactor 循环后，方可开始下一个类的循环。这确保每个类都有独立的、经过验证的 TDD 循环
...
- **P1-1** 【触发时机：每个 Story 开发前】**必须**先输出该 Story 的设计文档（包含 Story 概要、模块设计、状态机设计），然后**暂停执行**，等待用户确认后方可开始该 Story 的开发。Story 设计文档的命名和存放路径遵循 P2-3 交付件清单中"迭代开发"阶段的规范
...
| 步骤 | 名称 | 规则依据 | 关键动作 |
|------|------|---------|---------|
| **S1** | 开发前准备 | P0-13 | 读取 `MEMORY.md` 经验教训 → 输出 Story 概要（名称、目标、涉及文件/模块、关键实现要点、预估工作量） → **暂停等待用户确认** |
| **S1.5** | 输出 Story 设计文档 | P1-1 | `[Agent] godot-architect` 输出该 Story 的设计文档（模块设计 + 状态机设计），存放到 `docs/03_arch/`。**暂停等待用户确认设计文档** |
| **S2** | 梳理验收标准 | P0-12、P1-10 | 列出所有 BDD 场景（Given-When-Then），确保每条验收标准有对应测试用例。不够具体时**必须**先与用户澄清 |
| **S3** | 编写失败测试（红） | P0-11、P0-11.5 | `[Agent] godot-developer` 按类的依赖关系排序（依赖最少的类优先），每次**只编写一个类**的单元测试和集成测试（`test/unit/` + `test/integration/`），确保测试失败 |
| **S4** | 编写最小实现（绿） | P0-11、P0-11.5 | `[Agent] godot-developer` 为当前类写最少的代码使测试通过。使用 `[Skill] godot-best-practices` + `[Skill] godot-gdscript-patterns` 指导实现 |
| **S5** | 重构代码（重构） | P0-11、P0-11.5 | `[Agent] godot-developer` 对当前类优化代码结构，保持测试通过。遵循 SOLID + DRY 原则。完成后回到 S3 开始下一个类的 Red→Green→Refactor 循环，直至所有类完成 |
...
| 迭代开发 | Story 模块设计 | `{序号}_模块设计_{模块名}.md` | `docs/03_arch/` |
| 迭代开发 | Story 状态机设计 | `{序号}_状态机设计_{名称}.md` | `docs/03_arch/` |
