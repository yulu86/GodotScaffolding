...
| S5 | Red | `godot-developer` | **先输出测试用例表格再写测试代码**：① 对当前类按依赖拓扑顺序逐类输出测试用例表（列：场景分类 | 测试方法名 | 输入 | 期望输出），**必须**覆盖正常和异常场景 → **暂停等用户确认** ② 用户确认后按表格逐个编写测试代码，依赖最少优先；**同一依赖层级内无相互依赖的 task 可并行调度，并行度上限 4**（子代理执行，遵循 P0-17） |
