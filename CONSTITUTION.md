...
>
> **确认模式**（见 P1-26）：默认 `连续式`——S1→S2→S3→S4 文档准备阶段连续产出，仅在 S4 末尾统一确认一次；S15 代码检视为**硬性暂停点**，必须用户逐文件参与；S17 场景验收默认截图自动比对。用户可切换为 `交互式`（每个确认点单独暂停）。
...
| S15 | 代码检视 | `godot-reviewer` | 总结开发内容 → 在编辑器中逐个打开文件，说明职责、修改关键点、检视关键点 → **每检视完一个文件后展示检视进度**（示例：`检视进度: [███░░░░] 3/7`，`✓` 已完成、`→` 当前、其余为待检视） → **逐文件暂停等用户检视结果**（两模式均强制用户参与，不得自动放行，见 P1-26 硬性暂停点） |
...
- **P1-26** **确认模式**：Story 执行中 AI 暂停等用户的策略，默认 `连续式`，用户可随时切换为 `交互式`：
  - **连续式**：① S1→S2→S3→S4 文档准备阶段连续产出，仅在 S4 末尾统一确认一次；② S17 场景验收用截图自动比对，通过则不暂停
  - **交互式**：每个确认点（S1/S2/S3/S4/S8/S9/S15/S16/S17）均单独暂停等用户
  - **切换**：用户在任一暂停点或任务中说出"切换交互式/连续式"即生效；Story 开始时默认连续式
  - **硬性暂停点（两模式均必须暂停，不因连续式跳过）**：S8（需用户在编辑器完成可视化操作）、S9/S16（出现一致性差异时）、S15（代码检视须用户逐文件参与）、Story 间确认（P1-2）。安全/质量红线步骤不得自动放行
...
- **P1-6** S15 代码检视：AI 逐文件展示变更摘要，**必须**与用户一起检视（逐文件暂停，两模式均强制用户参与，不得自动放行，见 P1-26 硬性暂停点）
