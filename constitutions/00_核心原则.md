...
- **P1-12** 功能开发流程：
  ...
  task(category="deep", load_skills=["godot-ui-design"]) (界面设计，涉及 UI 时必须)
    ...
    → godot-scene：AI 创建场景基本框架 + 输出操作指导到 tmp/ (场景构建，串行执行，禁止子agent)
    → 用户在编辑器中完成可视化操作并确认
...
4. godot-scene      → AI 创建场景基本框架（节点树骨架）+ 输出操作指导到 tmp/
   ⚠️ 场景开发为严格串行操作，禁止在子agent 中执行
   ⚠️ 用户未确认完成前，禁止执行其他一切任务
...
> **注意**：当 Story 同时涉及场景和脚本开发时，**必须**优先完成场景开发（步骤4），再进行脚本开发（步骤5）。
...
6. **场景优先**：Story 同时涉及场景和脚本时，必须先完成场景开发再进行脚本开发
7. **场景串行**：场景开发为严格串行操作，用户未确认完成前禁止执行其他任务，场景开发禁止在子agent 中执行
