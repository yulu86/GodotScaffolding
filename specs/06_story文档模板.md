...
> **用途**：「阶段 6 · 需求文档」依据 Feature（`docs/05_需求/`）拆分产出；其后贯穿阶段 7~11 的开发与验收。
...
arch: docs/07_架构/01_xxx.md         # 关联架构方案（可选，阶段8 产出后补）
hifi: docs/04_高保真设计/01_xxx.md   # 关联高保真设计原型（可选，阶段5 产出后补）
---
```
...
| status | 含义 | 对应阶段 |
|--------|------|:--------:|
| `draft` | Story 草拟中，AC 未定稿 | 阶段 6 |
| `ready` | 需求冻结，AC 已定稿，可进入开发 | 阶段 6 完成 |
| `in-progress` | 开发进行中（架构→TDD→质量门禁） | 阶段 7~10 |
| `in-review` | 黑盒验收中，按 AC 逐条验证 | 阶段 11 |
| `done` | 所有 AC 验收通过，已交付 | 阶段 11 完成 |
...
> **强制可测**：每个 Scenario 都要能在阶段 11 被验证。
...
> 任务勾选对齐阶段 7~11；每个阶段完成后执行横切「收尾沉淀」（commit / MEMORY / qmd / 状态 / 飞书）。

- [ ] **阶段 7 · 启动准备**：查 `MEMORY.md` + LLM Wiki，确认可复用经验
- [ ] **阶段 8 · 架构设计**：产出架构方案（`docs/07_架构/`），方案通过评审后回填 frontmatter `arch`
- [ ] **阶段 9 · TDD 开发**：红 → 绿 → 重构（BDD 场景直接转化为测试用例）
  - [ ] 单元测试路径：`test/unit/...`（**必须镜像源码相对路径**）
  - [ ] 涉及脚本：`scripts/...`
  - [ ] 涉及场景：`scenes/...`（`.tscn` 用 MCP 生成，禁手写）
- [ ] **阶段 10 · 质量门禁**：`gdlint` / `gdformat` + GdUnit4 + code review（全过方可继续）
- [ ] **阶段 11 · 黑盒验收**：按上节 BDD 场景逐条验证，全过则 `status: done`
...
- **架构方案**：
- **Feature 文档**：
- **高保真设计原型**：（阶段5 HTML 原型，`docs/04_高保真设计/`，验证视觉 / 动效 / 交互；可关联到具体 BDD 场景）
- **涉及脚本 / 场景 / 资源**：
- **单元测试**：
