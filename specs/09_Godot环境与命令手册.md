...
| 14 | GdUnit4 headless 测试 | `$GODOT --headless -s res://addons/gdUnit4/bin/GdUnitCmdTool.gd -a <dir\|file> --ignoreHeadlessMode` | 8·9 |
...
**前提**：项目 `addons/gdUnit4` 已接入（v6.2.1，物理目录大小写为 `gdUnit4`，`project.godot` 注册路径 `res://addons/gdUnit4/plugin.cfg`）。**命令行用法以接入版本实测为准**。

```bash
# headless 跑测试：-a 直接接目录或文件路径（无 directory/suite 关键字）
# 跑整个目录（回归）
$GODOT --headless -s res://addons/gdUnit4/bin/GdUnitCmdTool.gd -a test/unit/ --ignoreHeadlessMode
# 跑单个测试套件（针对单 Story）
$GODOT --headless -s res://addons/gdUnit4/bin/GdUnitCmdTool.gd -a test/unit/scripts/autoloads/ui_state_machine_test.gd --ignoreHeadlessMode

# 常用选项（以 v6.2.1 --help 为准）
#   -a <dir|file.gd>           添加测试目标（目录=全跑，文件=单套件）
#   -c  / --continue           非失败即止，改为跑完全部用例（CI 推荐）
#   -i <testsuite[:test]>      忽略指定套件/用例
#   --ignoreHeadlessMode       关闭 headless 限制检查（见下方红字）
```

> ⚠️ **三个必踩坑（v6.2.1 实测）**：
> 1. **入口路径是 `bin/GdUnitCmdTool.gd`**（非 `addons/gdunit4/GdUnitCmdTool.gd`）；目录大小写为 **`gdUnit4`**（大写 U）。macOS 大小写不敏感只 WARNING，**Linux 大小写敏感会直接失败** → 路径必须原样照写。
> 2. **`-a` 不接 `directory`/`suite` 关键字**，直接接路径（旧写法 `-a directory <dir>` / `-a suite <file>` 会报 `Unknown ... command!` 退出 100）。
> 3. **`--headless` 默认被 GdUnit4 拒绝**（防 InputEvent 在 headless 下失效，退出码 103）。**纯逻辑测试加 `--ignoreHeadlessMode` 即可**；涉及 InputEvent 的测试（如 STORY-006 输入设备检测）在 headless 下 InputEvent 不传递，需改用编辑器内运行或非 headless 自动化方案。

> **退出码门禁**（GdUnitSessionRunner 常量，CI 必须用退出码判定，**禁止**靠输出文本猜测）：
> | 码 | 常量 | 含义 |
> |:--:|------|------|
> | 0 | `RETURN_SUCCESS` | 全部用例通过 |
> | 100 | `RETURN_ERROR` | 用例失败 / 参数错误 |
> | 101 | `RETURN_WARNING` | 存在 flaky |
> | 103 | `RETURN_ERROR_HEADLESS_NOT_SUPPORTED` | 未加 `--ignoreHeadlessMode` |
> | 104 | `RETURN_ERROR_GODOT_VERSION_NOT_SUPPORTED` | Godot 版本不受支持 |
> | 105 | `RETURN_ERROR_SCRIPT_ERRORS_DETECTED` | 测试脚本本身有语法错误 |
>
> 测试报告自动生成于 `reports/report_<N>/`（`results.xml` + `index.html`）。
...
| GdUnit4 报 `Unknown '...' command!` 退出 100 | `-a` 后误加了 `directory`/`suite` 关键字 → 直接接路径（见 4.2） |
| GdUnit4 报 `Headless mode is not supported!` 退出 103 | 默认禁 headless → 加 `--ignoreHeadlessMode`（纯逻辑测试，见 4.2） |
| GdUnit4 报 `Case mismatch ... gdunit4 ... gdUnit4` | 路径大小写写错 → 入口必须写 `addons/gdUnit4/bin/GdUnitCmdTool.gd`（Linux 会直接失败） |
